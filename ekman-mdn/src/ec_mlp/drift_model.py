...
class DriftModel(eqx.Module):
    ...
    def get_diffusivity_tensor_from_mdn_parameters(
        ...
        where:
            - \Delta X_t is the drift displacement over time interval \Delta t, 
            - f(t, X_t) is the deterministic drift velocity.
        The covariance can be decomposed as:
        $$
        Cov[\Delta X_t - f(t, X_t) | (t, X_t)] = 
            E[Cov[\Delta X_t - f(t, X_t) | (t, X_t)]] + 
            Cov[E[\Delta X_t - f(t, X_t) | (t, X_t)]]
        $$
        The first term is the expected covariance of the Gaussian components, 
        and the second term is the covariance of the means of the Gaussian components.
        $$
        E[Cov[\Delta X_t - f(t, X_t) | (t, X_t)]] = \sum_{k=1}^{n} \pi_k \Sigma_k
        $$
        and
        $$
    ...
    def get_alongflow_diffusivity(
        ...
        return self.get_alongflow_diffusivity_from_mdn_parameters(*mdn_parameters)
    
    def get_alongflow_diffusivity_from_mdn_parameters(
        self,
        pi_k: Float[jax.Array, "n_components"],
        mu_k: Float[jax.Array, "n_components 2"],
        sigma_k: Float[jax.Array, "n_components 2"],
