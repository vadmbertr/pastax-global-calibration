...
class _PhysicalField:
    def __init__(
        self, data: ZarrData, u_var: str, v_var: str, periodic_grid: bool, max_travel_distance: float, n_days: int
    ):
        self.u_var = u_var
        self.v_var = v_var
    ...
    def __call__(
        ...
        if self.periodic_grid:  # periodic global domain
            if lon_padleft != 0:
                ...
                u_left = patch_left[self.u_var]
                v_left = patch_left[self.v_var]
                lon_left = patch_left.longitude

                u = np.concat([u_left, u], axis=-1)
                v = np.concat([v_left, v], axis=-1)
            ...
            if lon_padright != 0:
                ...
                u_right = patch_right[self.u_var]
                v_right = patch_right[self.v_var]
                lon_right = patch_right.longitude

                u = np.concat([u, u_right], axis=-1)
                v = np.concat([v, v_right], axis=-1)
                lon = np.concat([lon, lon_right])

                lon_padright = 0

        u = np.pad(u, ((t_padleft, t_padright), (lat_padleft, lat_padright), (lon_padleft, lon_padright)), mode="edge")
        v = np.pad(v, ((t_padleft, t_padright), (lat_padleft, lat_padright), (lon_padleft, lon_padright)), mode="edge")
...
class XarrayDataset(TorchDataset):
    def __init__(
        ...
        self.uc = _PhysicalField(uc, "ugos", "vgos", periodic_grids, max_travel_distance, n_days)
        self.uw = _PhysicalField(uw, "eastward_wind", "northward_wind", periodic_grids, max_travel_distance, n_days)
        self.uh = _PhysicalField(uh, "VSDX", "VSDY", periodic_grids, max_travel_distance, n_days)
