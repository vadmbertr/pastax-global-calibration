...
class StochasticModule(L.LightningModule):
    def __init__(
        ...
        self.antithetic_variate = antithetic_variate
    ...
    def training_step(self, batch: tuple[list[torch.Tensor], list[torch.Tensor]]) -> torch.Tensor:
        ...
        random_variables_batch = self._get_random_variables(batch_size)
        ...
        self.dynamics, self.opt_state, loss, grads, metrics = self.make_step(
            ...
            self.dynamics, self.opt_state, grids_batch, random_variables_batch, reference_trajectory_batch, indices
        )
        ...
        mu, sigma = self.dynamics.get_coefficients()
        
        ekman_scale, ekman_rotation, stokes_scale, leeway_scale = mu
        ekman_scale_var, ekman_rotation_var, stokes_scale_var, leeway_scale_var = jnp.diag(sigma)
        ...
        self.log_dict(
            {
                "coef_var_ekman_scale": ekman_scale_var.item(),
                "coef_var_ekman_rotation": ekman_rotation_var.item(),
    ...
    def validation_step(self, batch: tuple[list[torch.Tensor], list[torch.Tensor]]) -> torch.Tensor:
        ...
        _, (loss, metrics) = self._batch_step(
            ...
            grids_batch, random_variables_batch, reference_trajectory_batch, indices
        )
    ...
    def forward(
        ...
        return self._forward(
            self.simulator, self.integration_dt, self.n_steps, self.dynamics, grids, random_variables, x0, ts
        )
    ...
    def _batch_step(
        ...
        loss, metrics = eqx.filter_vmap(
            lambda grid, random_variables, reference_trajectory: self._sample_step(
                dynamics, 
                simulator, 
                integration_dt, 
                n_steps, 
    ...
    def _sample_step(
        ...
        simulated_trajectories = self._forward(
            simulator, integration_dt, n_steps, dynamics, grids, random_variables, x0, ts
        )
    ...
    def _forward(
        ...
        simulated_trajectories = eqx.filter_vmap(
            lambda random_var: simulator(
                dynamics=dynamics, args=(*grids, (random_var, ts[0], integration_dt)), x0=x0, ts=ts, solver=dfx.Tsit5(),
                dt0=dt0, saveat=saveat, stepsize_controller=stepsize_controller, adjoint=adjoint, max_steps=n_steps
            )
        )(random_variables)

        simulated_trajectories = TrajectoryEnsemble.from_array(
            simulated_trajectories.locations.value, ts, unit=x0.unit
        )
