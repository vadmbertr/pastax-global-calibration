...
class DeterministicModule(L.LightningModule):
    def __init__(
        ...
        self.save_hyperparameters("integration_dt", "learning_rate_scheduler", "optimizer", "learning_rate")
    ...
    def training_step(self, batch: tuple[list[torch.Tensor], list[torch.Tensor]]) -> torch.Tensor:
        ...
        if not all_finite:
            ...
            simulated_trajectory_batch = eqx.filter_vmap(
                lambda grids, reference_trajectory: self.forward(
                    grids, reference_trajectory.origin, reference_trajectory.times.value
                )
            )(grids_batch, reference_trajectory_batch)

            debug_loss = eqx.filter_vmap(
                lambda reference_trajectory, simulated_trajectory: self._separation_distance(
                    reference_trajectory, simulated_trajectory, indices
                )
            )(reference_trajectory_batch, simulated_trajectory_batch)
            ...
            print(f"sim end min: {simulated_trajectory_batch.locations.value[highest_loss, -1]}")
            print(f"sim end mean: {simulated_trajectory_batch.locations.value[highest_loss, -1]}")
            print(f"sim end max: {simulated_trajectory_batch.locations.value[highest_loss, -1]}")

            def _steps_velocities(traj: Trajectory) -> tuple[Float[Array, "traj_len"], Float[Array, "traj_len"]]:
                steps = traj.steps().value
                times = traj.times.value[1:] - traj.times.value[:-1]
                times = jnp.pad(times, (1, 0), constant_values=1e-4)

                return steps, steps / times
            
            steps, velocities = eqx.filter_vmap(_steps_velocities)(simulated_trajectory_batch)

            print(f"max sim step: {steps[highest_loss].max()}")
            print(f"max sim velocity: {velocities[highest_loss].max()}")
