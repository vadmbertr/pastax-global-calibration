...
def plot_fn(data_path: str, trainer: L.Trainer, trainer_module: L.LightningModule):
    ...
    beta_e, theta_e, beta_w, u_eps, v_eps = eqx.filter_vmap(
        lambda arr: drift_model.get_physical_parameters(arr[0], arr[1], arr[2], in_degrees=True)
    ...
    u_eps_map = u_eps.reshape(len(months), len(lats), len(lons))
    v_eps_map = v_eps.reshape(len(months), len(lats), len(lons))
    ...
    make_plots("theta_e", theta_e_map, cmap=cmo.balance, vmin=-90, vmax=90, unit="°")
    make_plots("beta_w", beta_w_map, cmap=cmo.amp, vmin=0, vmax=None, unit="%")
    make_plots("u_eps", u_eps_map, cmap=cmo.diff, vmin=-0.25, vmax=0.25, unit="m/s")
    make_plots("v_eps", v_eps_map, cmap=cmo.diff, vmin=-0.25, vmax=0.25, unit="m/s")
